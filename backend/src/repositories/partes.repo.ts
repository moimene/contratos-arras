...
export async function createParte(data: Partial<Parte>): Promise<Parte> {
    ...
    const parte = {
        id,
        rol: data.rol!,
        nombre: data.nombre || (data as any).denominacion!,
        apellidos: data.apellidos || '',
        estado_civil: data.estado_civil || null,
        tipo_documento: data.tipo_documento || ((data as any).cif ? 'CIF' : 'DNI'),
        numero_documento: data.numero_documento || (data as any).cif!,
        email: data.email || (data as any).representante?.email!,
        telefono: data.telefono || null,
        domicilio: data.domicilio || (data as any).domicilio_social || null,
